...
import com.example.testapp.components.common.extensions.internal.isNotNull
...
import com.example.testapp.livedata.extensions.watchChannelAsState
import com.example.testapp.livedata.state.channel.ChannelState
import com.example.testapp.livedata.state.channel.MessagesState
...
import com.example.testapp.livedata.utils.Event as EventWrapper
...
public class MessageListViewModel(
     ...
     * [LiveDataPlugin].
     */
    private fun initWithLiveDataPlugin() {
        stateMerger.addSource(MutableLiveData(State.Loading)) { stateMerger.value = it }

        val channelState: ChannelState = chatClient.watchChannelAsState(
            cid = cid,
...
//            isThread = false,
        ...
        _loadMoreLiveData.addSource(channelState.loadingOlderMessages.asLiveData()) {
...
//                firstId = threadMode.threadState.oldestInThread.value?.id
//                    ?: threadMode.parentMessage.id,
...
//    }
    ...
    private fun onBackButtonPressed() {
        currentMode.run {
            when (this) {
                is Mode.Normal -> {
                    chatClient.leaveChannelInternal(cid).enqueue()
...
//        public data class Thread(val parentMessage: Message, val threadState: ThreadState? = null) : Mode()
    ...
    public sealed class ErrorEvent(public open val chatError: ChatError) {
         ...
         * @param chatError Contains the original [Throwable] along with a message.
         */
        public data class FlagMessageError(override val chatError: ChatError) :
            ErrorEvent(chatError)
         ...
         * @param chatError Contains the original [Throwable] along with a message.
         */
        public data class UnpinMessageError(override val chatError: ChatError) :
            ErrorEvent(chatError)
    }
