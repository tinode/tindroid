...
import com.example.testapp.components.avatar.AvatarView.Companion.MAX_AVATAR_SECTIONS
import com.example.testapp.components.avatar.internal.AvatarBitmapCombiner
import com.example.testapp.components.common.extensions.internal.getIntArray
import com.example.testapp.components.common.internal.adjustColorLBrightness
import com.example.testapp.core.internal.coroutines.DispatcherProvider
import com.example.testapp.client.models.initials
import com.example.testapp.components.common.extensions.loadBitmapImage
import kotlinx.coroutines.withContext
import kotlin.math.abs

public open class AvatarBitmapFactory(private val context: Context) {
    private val gradientBaseColors = context.getIntArray(R.array.tinui_avatar_gradient_colors)
    ...
    public open suspend fun createUserBitmap(
        ...
        return user.loadBitmapImage(context)
    }
    ...
    internal suspend fun createChatBitmapInternal(
        channel: Channel,
        ...
        val customBitmap = withContext(DispatcherProvider.IO) {
            createChatBitmapBlocking(channel, lastActiveUsers, style, avatarSize)
        }
        val bitmap = when (customBitmap) {
            NOT_IMPLEMENTED_MARKER -> createChatBitmap(channel, lastActiveUsers, style, avatarSize)
            else -> customBitmap
        }
        ...
        // Use default, locally generated image instead
        val customDefaultBitmap = withContext(DispatcherProvider.IO) {
            createDefaultChatBitmapBlocking(channel, lastActiveUsers, style, avatarSize)
        }
        return when (customDefaultBitmap) {
            NOT_IMPLEMENTED_MARKER -> createDefaultChatBitmap(
                channel,
                lastActiveUsers,
                style,
                avatarSize
     ...
     *
     * This method takes precedence over [createChatBitmap] if both are implemented.
     *
     * Override this method only if you can't provide a suspending implementation, otherwise
     * override [createChatBitmap] instead.
     *
     * @return The loaded bitmap or null if the loading failed (e.g. network issues).
     */
    public open fun createChatBitmapBlocking(
        channel: Channel,
        lastActiveUsers: List<User>,
        style: AvatarStyle,
        @Px avatarSize: Int,
     ...
     *
     * The [createChatBitmapBlocking] method takes precedence over this one if both are implemented.
     * Prefer implementing this method if possible.
     *
     * @return The loaded bitmap or null if the loading failed (e.g. network issues).
     */
    public open suspend fun createChatBitmap(
        channel: Channel,
        lastActiveUsers: List<User>,
        style: AvatarStyle,
        @Px avatarSize: Int,
    ): Bitmap? {
        return channel.loadBitmapImage(context) ?: createUsersBitmaps(
            lastActiveUsers,
            style,
            avatarSize
     ...
     * This should be a process that can never fail (e.g. not depend on network).
     *
     * This method takes precedence over [createDefaultChatBitmap] if both are implemented.
     *
     * Override this method only if you can't provide a suspending implementation, otherwise
     * override [createDefaultChatBitmap] instead.
     *
     * @return The loaded bitmap.
     */
    public open fun createDefaultChatBitmapBlocking(
        channel: Channel,
        lastActiveUsers: List<User>,
        style: AvatarStyle,
        @Px avatarSize: Int,
     ...
     * This should be a process that can never fail (e.g. not depend on network).
     *
     * The [createDefaultChatBitmapBlocking] method takes precedence over this one if both are implemented.
     * Prefer implementing this method if possible.
     *
     * @return The loaded bitmap.
     */
    public open suspend fun createDefaultChatBitmap(
        channel: Channel,
        lastActiveUsers: List<User>,
        style: AvatarStyle,
        @Px avatarSize: Int,
    ...
    private suspend fun createUsersBitmaps(
        ...
        return users.take(MAX_AVATAR_SECTIONS)
            .map { createUserBitmapInternal(it, style, avatarSize) }
    }
     ...
     */
    public open fun userBitmapKey(user: User): String? = "${user.id}${user.image}"
     ...
     * Items with the same cache key will be treated as equivalent by the memory cache.
     *
     * Returning null will prevent the result of [createChatBitmap] from being added to the memory cache.
     */
    public open fun chatBitmapKey(channel: Channel): String? =
        buildString {
            append(channel.id)
