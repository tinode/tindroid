...
import com.example.testapp.components.common.extensions.combineWith
...
//        }
    ...
    private fun addReads(
        ...
        val sortedReads = reads.filter { it.user.id != currentUserId && it.lastRead != null }.sortedBy { it.lastRead }.toMutableList()
        ...
        msgLoop@ for ((i, messageItem) in messages.reversed().withIndex()) {
            if (sortedReads.isEmpty()) {
                break@msgLoop
            }
            if (messageItem is MessageListItem.MessageItem) {
                messageItem.message.seq?.let { seq ->
                    readLoop@ while (sortedReads.isNotEmpty()) {
                        // use the list of sorted reads
                        val last = sortedReads.last()
                        if (seq <= last.lastRead!!) {
                            // we got a match
