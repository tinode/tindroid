...
internal class MessageListScrollHelper(
    ...
    private val layoutManager: LinearLayoutManager
        get() = requireNotNull(recyclerView.layoutManager?.safeCast<LinearLayoutManager>()) {
            "The RecyclerView need to initialized with LinearLayoutManager"
        }
        ...
        recyclerView.addOnScrollListener(
            object : RecyclerView.OnScrollListener() {
                ...
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                    ...
                    val lastPotentiallyVisibleItemPosition =
                        currentList.indexOfLast { it.isValid() }
        ...
        recyclerView.postDelayed(
            {
                currentList.indexOfFirst { it is MessageListItem.MessageItem && it.message.id == message.id }
                    ...
                    ?.let {
                        if (message.pinned) {
                            layoutManager.scrollToPositionWithOffset(it, 8.dpToPx())
                        } else {
                            with(recyclerView) {
                                this@MessageListScrollHelper.layoutManager.scrollToPositionWithOffset(
                                    it,
                                    height / 3
                                )
                                post {
...
//        isThreadStart: Boolean,
    ...
    private fun getLastSeenItemPosition(): Int {
        val lastMessageId = lastSeenMessageInChannel?.getStableId()
